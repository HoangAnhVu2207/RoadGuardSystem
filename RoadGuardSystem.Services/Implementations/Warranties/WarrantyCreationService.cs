...
{
    private readonly IWarrantyRepository _persistence;

    public WarrantyCreationService(IWarrantyRepository persistence)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var facts = await _persistence.GetFactsAsync(
            ...
            cancellationToken);
        if (facts.ProjectStatus is null)
        {
            return new(WarrantyCreationStatus.ProjectNotFound);
        }

        if (facts.ProjectStatus == ProjectStatus.Closed)
        {
            return new(WarrantyCreationStatus.ProjectClosed);
        }

        if (!facts.RoadSectionMatchesProject)
        {
            return new(WarrantyCreationStatus.RoadSectionNotFound);
        }

        if (!facts.HandoverDocumentMatchesProject)
        {
            return new(WarrantyCreationStatus.HandoverDocumentNotFound);
        }

        if (!facts.SourceDocumentExists)
        {
            return new(WarrantyCreationStatus.SourceDocumentNotFound);
        }
