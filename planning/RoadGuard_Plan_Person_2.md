...
| ID / size | Trace and dependency | Work and concrete output | Required tests and evidence |
|---|---|---|---|
| `P2-10` / 1.5d | US-01, CN01-CN03/CN10; P2-00 | Map/migrate User, Role, Session, RefreshToken, PasswordResetLog and AccountStatusChangeLog; seed four roles. Output: entities/configurations/migration/seed. | Negative: duplicate username/token hash, invalid status/log transition, plaintext token/password scan. Positive: session cascade/revocation queries and append-only logs persist on SQL Server. |
| `P2-11` / 1d | US-01, TE-03; P2-10 | Map project membership authorization read model and add integration/API security matrix fixture. Output: membership query, reusable users/tokens for four roles. | Negative: forged claim and cross-project IDs return 403/no rows. Positive: each role sees only assigned scope; Supervisor behavior explicitly tested. |
