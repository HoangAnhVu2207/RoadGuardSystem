...
| `P1-12` | `Done` | `anh` | Current-user/project authorization policy, reusable scope guard and DTO-only offline work-package read completed 2026-09-21. API build passed with zero warnings/errors; focused SQL-backed API tests passed 4/4 with zero failures/skips; real-host unauthenticated smoke returned stable `401 auth_unauthorized`; independent read-only review found no remaining issues. No schema, migration, package, commit, merge, push, deployment or publication. |
| `P1-20` | `Done` | `anh` | API/SQL verification completed 2026-09-21 on `anh` at `2fb2d54`: focused authenticated API/SQL tests passed 9/9 with zero failures/skips, covering project creation/update, warranty, authorization, audit, idempotency and concurrency. Supplemental P1-72 unit boundary/session checks passed 30/30; P1-72 remains In Progress for its separate hosted-CI/timing/integration handoff. See [P1-20 worklog](../docs/worklogs/P1-20-completion.md). |
| `P1-21` | `Done` | `anh` | RoadSection initial/version APIs completed 2026-09-21. Focused authenticated API/SQL tests passed 5/5, including replay, SRID validation, authorization, stale-version and concurrent-write conflicts; P2-21 SQL regression passed 6/6 and source-boundary checks passed 3/3. No schema, migration or package change. See [P1-21 worklog](../docs/worklogs/P1-21-completion.md). |
| `P1-72` | `In Progress` | `anh` | Team-lead-approved umbrella implementation for A0, S1-S6 and paired P1-77/P2-73 through P1-85/P2-81 boundaries. No schema or migration changes. Source builds; runtime, SQL, timing, and hosted-CI verification remain pending Huy's test handoff. |
