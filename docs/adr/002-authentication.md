...
- **Downstream Context:** Architectural foundation for use cases CN01 (Login), CN02 (Logout), CN03 (Change Password), CN10 (Account Suspension), and User Story US-01. (Note: P1-02 establishes architectural policy and contracts only; production code and endpoint implementation are assigned to P1-10, P2-10, and P1-12).
...
2. **Dual-Token OAuth 2.0 / Bearer Model:**
...
1. **Configurable Short-Lived Access Token:**
   ...
   - The access token payload contains standard RFC 7519 claims:
     ...
     - `role`: User role claims embedded for client UI rendering convenience only.
...
3. **Per-Request Session and Account Validation (Instant Revocation):**
   - Every incoming authenticated API request must be intercepted by authentication/authorization middleware to verify that:
     1. The session identified by the token's `sid` claim is currently active and not revoked (`revoked_at IS NULL`).
     2. The associated user account is active and has not been suspended (`status != SUSPENDED`).
   - If session validation fails, the request must immediately terminate with HTTP 401 Unauthorized and standard error code `auth_session_revoked` or `auth_unauthorized`.

4. **Instant Revocation Triggers:**
   - The following operations must immediately block subsequent API requests:
     - **User Logout (CN02):** Sets `revoked_at = UtcNow` for the current session.
     - **Password Reset / Change (CN03):** Sets `revoked_at = UtcNow` across **all active sessions** belonging to the user and records an append-only entry in `PasswordResetLog`.
     - **Account Suspension (CN10):** Sets `revoked_at = UtcNow` across all active sessions and refresh tokens for the user and records an append-only entry in `AccountStatusChangeLog`.
     - **Replay Attack Detection:** Immediately revokes the entire session and token family (detailed below).

5. **Caching & Fail-Closed Strategy:**
   - To avoid redundant database queries on high-throughput endpoints, session and account validity may be cached in an in-memory or distributed cache.
   - Cache invalidation must be instantaneous and event-driven upon any logout, password change, suspension, or token rotation event.
   - Under any cache ambiguity, communication failure, or timeout, the system must **fail closed** (deny the request and return HTTP 401).
...
1. **Multi-Tenancy and Civil Infrastructure Scope:**
   - RoadGuard aggregates (Surveys, Road Sections, Defects, Repair Items, Warranties) are strictly bound to specific civil infrastructure `Project` entities.
   - Authorization cannot be determined solely by static role claims (e.g., `role: Inspector` or `role: ProjectManager`).
...
|---|---|---|---|
| **Domain** | `BusinessObjects` | Domain user and role models (`ApplicationUser`, `ApplicationRole`), session entities, enum definitions (`UserRole`, `AccountStatus`). | P1-00 / P1-11 |
...
| **Presentation** | `API` | JWT Bearer authentication handler configuration, token extraction, correlation middleware, and thin controllers (`AuthController`). | P1-01 / P1-10 |
...
2. **Secret Configuration:**
   - JWT symmetric signing keys (`SecretKey`), database credentials, and external service keys must be supplied through ASP.NET Core configuration, environment variables, or secure secret vaults.
...
3. **Log Sanitization and Sensitive Data Suppression:**
   ...
   - The following sensitive data elements must **NEVER** be recorded in application logs, audit logs, or error responses:
     ...
     - Symmetric signing keys or database connection strings.
...
1. **Google OAuth / External SSO:**
   ...
   - System specifications (`Dac_ta_UseCase_v2.md` CN01–CN03 and `User_Stories_Acceptance_Criteria_v2.md` US-01) specify internal username/email and password credentials only.
...
1. **Pure Stateless JWT without Session Verification:**
   - *Rejected:* Without server-side session checks, access tokens remain valid until expiration even after user logout, account suspension, or password reset. This security vulnerability violates core domain requirements.
...
### Positive
- **Instantaneous Revocation:** Suspended accounts and compromised tokens are terminated immediately across all clients.
...
- **Auditability:** Complete, append-only history of password changes and account suspensions.
...
1. **Distributed Cache Selection:** Evaluation of distributed cache backend (Redis vs. SQL Server cache vs. in-memory) for session lookup caching will be finalized during production infrastructure deployment.
2. **Multi-Factor Authentication (MFA):** TOTP/SMS-based two-factor authentication is deferred to future project releases.
