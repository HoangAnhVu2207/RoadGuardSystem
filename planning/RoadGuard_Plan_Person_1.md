...
| `P1-02` / 0.5d | Architecture, after P1-00 | Create ADRs for backend boundary and authentication choice; define the API error-code naming policy. Output: `docs/adr/001-backend-boundary.md`, `002-authentication.md`, `docs/api-errors.md`. | Negative review: ADR lists rejected alternatives and unresolved decisions. Positive: links and decisions are internally consistent; markdown check passes if configured. |

## Wave 1 — identity and access

| ID / size | Trace and dependency | Work and concrete output | Required tests and evidence |
|---|---|---|---|
| `P1-10` / 1.5d | US-01, CN01-CN03; P2-10 | Implement login/refresh/logout and forced password change application flow with hashed refresh tokens. Output: Auth DTOs, service interfaces/implementation, endpoints, error codes. | Negative: null/empty credentials, suspended/pending account, wrong password, expired/revoked/replayed refresh token. Positive: login→refresh→logout; old token unusable. |
...
| `P1-12` / 1d | US-01, TE-03; P2-11 | Add current-user/project authorization service and endpoint policies. Output: authorization handlers/policies and reusable project-scope guard. | Negative: unauthenticated 401, wrong role/project 403, forged project claim. Positive: assigned member and Supervisor paths; no query runs before scope check where measurable. |
...
| `P1-64` / 2d | US-17, CN04, QT01-QT05; P2-64 | Implement Admin account suspend/reactivate with open-work handover list, notification inbox, defect catalog/severity-rule versioning and reminder-rule APIs. Output: admin/notification/config services, DTOs and endpoints. | Negative: non-Admin, suspend already suspended user, self-escalation, mutate active rule version, reminder auto-creates survey, sync draft transfer. Positive: suspend revokes sessions, preserves history/drafts, lists reassignment work and publishes versioned rules/reminders. |
