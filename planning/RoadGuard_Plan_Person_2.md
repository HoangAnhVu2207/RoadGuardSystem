...
| `P2-01` | `Done` | `huy` | Independent Codex remediation acceptance recorded 2026-09-19 for fix `82ca913`: dedicated database migration before two Seeder invocations; exact-SHA hosted runs `35390521534` (`huy`) and `35390565841` (`develop`) succeeded through security, tests, Seeder, coverage and cleanup. Local reviewer machine lacks SDK `10.0.401`; matching-content submission and hosted evidence satisfy the gate. Production Seeder and accepted P2-10 artifacts remain unchanged. See P2-01 worklog. |
| `P2-04` | `Done` | `huy` | Independent Codex acceptance Round 2 recorded for manifest `61f4ddbb...8322`; F-01 `[P1]` is Verified. Reviewer reruns passed symlink 5/5, P2-04 real SQL 34/34, non-incremental build and format; matching controlled full evidence is 380/380 with no skips. Local acceptance only; no commit, integration, push or deployment. See P2-04 worklog. |
| `P2-05` | `Done` | `huy` | DefectType, CauseCategory and SeverityRuleVersion schema/fixtures completed with owner-approved minimal Defect consumer shell and restrict-delete FKs. Affected IntegrationTests passed 220/220 with zero skips; repository/test builds and format verification passed. No commit, integration, push or deployment evidence is implied by this status row. |
| `P2-06` | `Done` | `huy` | DroneDevice registry schema, idempotent synthetic seed boundary and SQL fixtures completed before Flight. Affected IntegrationTests passed 224/224 with zero skips; repository/test builds and format verification passed. No commit, integration, push or deployment evidence is implied by this status row. |
| `P2-07` | `Done` | `huy` | Notification schema and deduplicated outbox-consumer persistence boundary completed. Affected IntegrationTests passed 229/229 with zero skips; repository/test builds and format verification passed. No commit, integration, push or deployment evidence is implied by this status row. |
| `P2-11` | `Done` | `huy` | Authoritative active/effective ProjectMember role read model and four-role SQL fixture completed. Focused P211 IntegrationTests passed 5/5 with zero skips; repository/test builds and format verification passed. No commit, integration, push or deployment evidence is implied by this status row. |
| `P2-21` | `Done` | `huy` | Owner-approved Option B road/version schema, Warranty persistence and SQL transaction boundary completed. Affected IntegrationTests passed 240/240 with zero skips; repository/test builds and format verification passed. No commit, integration, push or deployment evidence is implied by this status row. |
| `P2-22` | `Done` | `huy` | SurveyPlan, SurveyRequest and append-only postponement schema completed with canonical request states, SQL scope constraints, filtered active uniqueness and migration downgrade/reapply evidence. Per approved Data Dictionary interpretation, Plan/Request retain `RoadSectionId`; the `RoadSectionVersion` anchor belongs to Survey in P2-30. Affected IntegrationTests passed 248/248 with zero skips; focused P2-22 integration tests passed 8/8 and unit tests passed 6/6. Repository, integration and unit format verification passed. No commit, integration, push or deployment evidence is implied by this status row. |
| `P2-23` | `Done` | `huy` | Survey and SurveyAssignment schema completed with version/request scope triggers, baseline and assignment-history SQL constraints, filtered active-assignment uniqueness, and migration downgrade/reapply evidence. Reassignment atomically preserves history with allow-listed audit and one outbox event; durable idempotency replay includes post-commit acknowledgement recovery, while P2-07 consumer deduplicates the notification effect. Focused P2-23 IntegrationTests passed 7/7 and unit tests passed 3/3; affected IntegrationTests passed 259/259 with zero skipped. Repository build, format, whitespace and pending-model checks passed. No commit, push or deployment evidence is implied by this status row. |
| `P1-72` | `In Progress` | `anh` | Team Lead assigned the cross-owner structural/tooling implementation to the `anh` branch. Existing Done evidence remains unchanged; Huy owns the deferred SQL, timing, container, and hosted-CI verification handoff. |
